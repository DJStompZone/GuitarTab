#!/usr/bin/env python3
"""
TAB-focused / constrained-decoding-aligned training (Hydra).

Copy of train.py with optional loss_mode (tab_only, tab_restricted, composite).
Standard training remains: python train.py
"""
...
from src.tab_dataset import TabDataset, TabDatasetBatchInput, Vocabulary
...
from src.training_loss import compute_tab_focused_loss
...
def _load_init_checkpoint(
    ...
    """Load model weights only (finetune). Call after model.to(device), before optimizer."""
    ckpt = torch.load(path, map_location=device)
    state = ckpt.get("model_state_dict", ckpt)
    model.load_state_dict(state, strict=strict)
    ep = ckpt.get("epoch", "?")
    print(f"Loaded init_checkpoint (weights only) from {path} (source epoch {ep})")
...
def _load_resume_checkpoint(
    ...
    """Load model + optimizer; continue from next epoch. Returns (start_epoch, best_val_loss)."""
    ckpt = torch.load(path, map_location=device)
    ...
    print(
        f"Resumed from {path}: next epoch {start_epoch}, "
        f"checkpoint val_loss={ckpt.get('val_loss', 'n/a')}"
    )
    return start_epoch, best_val_loss


def _num_frets_for_tab_loss(cfg: DictConfig) -> int:
    """Resolve training.tab_loss_num_frets; fall back to data.num_frets if unset or non-numeric."""
    v = cfg.training.get("tab_loss_num_frets", None)
    try:
        if v is None:
            return int(cfg.data.num_frets)
        return int(v)
...
def train_epoch(
    ...
    for step, batch in enumerate(pbar):
        ...
        loss_mode = str(cfg.training.get("loss_mode", "full"))
        num_frets = _num_frets_for_tab_loss(cfg)
        comp_lam = float(cfg.training.get("composite_lambda", 0.5))
...
def evaluate(
    ...
    """Evaluate model (teacher-forcing loss; respects training.loss_mode)."""
    model.eval()
    total_loss = 0
    num_batches = 0
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(val_loader, desc="Evaluating"):
            ...
            if loss_mode == "full":
                ...
                batch_loss = compute_tab_focused_loss(
                    outputs.logits,
                    labels_shifted=labels,
                    decoder_input_ids=decoder_input_ids,
                    output_vocab=output_vocab,
...
@hydra.main(version_base=None, config_path="configs", config_name="train_tab_constrained")
def main(cfg: DictConfig):
    """Main training function (TAB-focused loss modes)."""
    print("=" * 80)
    print("Fretting-Transformer Training (TAB-constrained loss)")
    print("=" * 80)
    print(f"training.loss_mode = {cfg.training.get('loss_mode', 'full')}")
    # print("\nConfiguration:")
    ...
    init_ckpt = cfg.training.get("init_checkpoint") or None
    resume_ckpt = cfg.training.get("resume_checkpoint") or None
    if init_ckpt and resume_ckpt:
        raise ValueError("Use only one of training.init_checkpoint or training.resume_checkpoint")

    if init_ckpt:
        p = Path(str(init_ckpt)).expanduser()
        if not p.is_file():
            raise FileNotFoundError(f"init_checkpoint not found: {p}")
        strict = bool(cfg.training.get("init_checkpoint_strict", True))
        _load_init_checkpoint(model, p, device, strict=strict)
    ...
    if resume_ckpt:
        p = Path(str(resume_ckpt)).expanduser()
        if not p.is_file():
            raise FileNotFoundError(f"resume_checkpoint not found: {p}")
        start_epoch, best_val_loss = _load_resume_checkpoint(model, optimizer, p, device)
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs + 1):
        ...
        val_loss = evaluate(
            model, val_loader, device, output_vocab=dataset.output_vocab, cfg=cfg
        )
    ...
    test_loss = evaluate(
        model, test_loader, device, output_vocab=dataset.output_vocab, cfg=cfg
    )
